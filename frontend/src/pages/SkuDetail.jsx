        ...
        <>
          ...
          <div className="panel">
            ...
            <div className="kv">
              ...
              <span>→</span>
              <span className="badge badge-ftype">Magento: {detail.data.magento_type}</span>
              <StatusBadges hasIssues={detail.data.has_issues} issueCats={detail.data.issue_cats} />
            </div>
            <div className="kv">
              {detail.data.found_in_magento
                ? <span className="badge badge-ok">✅ Found in Magento</span>
                : <span className="badge badge-err">❌ Missing in Magento</span>}
            </div>
          </div>
      ...
      <div className="panel">
        ...
        {tree.data && (
          <div className="row-gap" style={{ alignItems: 'flex-start', gap: 18 }}>
            ...
            <div style={{ flex: 1, minWidth: 280 }}>
              <div className="muted" style={{ marginBottom: 6, fontWeight: 600 }}>Magento</div>
              <pre className="tree">{tree.data.magento.join('\n') || '—'}</pre>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="panel">
      ...
      <div className="kv">
        ...
        <span>→</span>
        <span className="badge badge-ftype">Magento: {tc.magento_type}</span>
  ...
  return (
    <div className="panel">
      ...
      <div className="kv">
        ...
        <span>·</span>
        <span>Magento slots: <b>{bc.magento_slot_count}</b></span>
        <MatchBadge ok={bc.slot_count_match} />
      </div>
      {bc.slots_only_in_pim.length > 0 && (
        <div className="kv"><span>❌ Missing in Magento:</span><SkuChips skus={bc.slots_only_in_pim} variant="missing" /></div>
      )}
      {bc.slots_only_in_magento.length > 0 && (
        <div className="kv"><span>ℹ️ Extra in Magento:</span><SkuChips skus={bc.slots_only_in_magento} variant="extra" /></div>
      )}
      {Object.entries(bc.per_slot).map(([slot, sd]) => (
        <div className="slot-box" key={slot}>
          <div className="kv">
            ...
            <span>·</span>
            <span>Magento: <b>{sd.magento_sku_count}</b></span>
          ...
          {sd.skus_only_in_magento.length > 0 && (
            <div className="kv"><span>ℹ️ Extra:</span><SkuChips skus={sd.skus_only_in_magento} variant="extra" /></div>
          )}
          {sd.count_match && sd.skus_only_in_pim.length === 0 && sd.skus_only_in_magento.length === 0 && (
  ...
  return (
    <div className="panel">
      ...
      <div className="kv">
        ...
        <span>·</span>
        <span>Magento: <b>{cc.magento_child_count}</b></span>
        <MatchBadge ok={cc.count_match} />
      </div>
      {cc.skus_only_in_pim.length > 0 && (
        <div className="kv"><span>❌ Missing in Magento:</span><SkuChips skus={cc.skus_only_in_pim} variant="missing" /></div>
      )}
      {cc.skus_only_in_magento.length > 0 && (
        <div className="kv"><span>ℹ️ Extra in Magento:</span><SkuChips skus={cc.skus_only_in_magento} variant="extra" /></div>
      )}
      {cc.unresolved_magento_ids?.length > 0 && (
        <p className="muted">⚠️ Unresolved Magento IDs: {cc.unresolved_magento_ids.join(', ')}</p>
      )}
      {cc.count_match && cc.skus_only_in_pim.length === 0 && cc.skus_only_in_magento.length === 0 && (
        <span className="badge badge-ok">✅ All children match</span>
      )}
    </div>
  )
