...
def batch_compare(pim_map: dict, magento_map: dict, id_to_sku: dict) -> list:
    """Run compare_product() for every SKU in PIM."""
    return [
        compare_product(sku, pim_map, magento_map, id_to_sku)
        for sku in sorted(pim_map)
    ]

def batch_compare_html(pim_map: dict, magento_map: dict, id_to_sku: dict, output_path: str = "batch_report.html") -> str:
    """Run batch comparison and generate HTML report."""
    results = batch_compare(pim_map, magento_map, id_to_sku)
