...
def compare_product(
    ...
    storefront_entry = storefront_map.get(pim_sku)

    report = {
        "sku":                     pim_sku,
        "found_in_pim":            pim_entry    is not None,
        "found_in_storefront":         storefront_entry is not None,
    ...
    report["type_comparison"] = compare_types(pim_entry, storefront_entry)

    pt = pim_entry.get("product_type")
    ft = (storefront_entry or {}).get("type_id")

    if pt == "BUNDLE" and ft == "bundle":
        report["bundle_comparison"] = compare_bundle_slots(pim_entry, storefront_entry)

    elif pt == "PRODUCT" and ft == "configurable":
        report["configurable_comparison"] = compare_configurable_children(
            pim_entry, storefront_entry, id_to_sku
        )

    return report

def batch_compare(pim_map: dict, storefront_map: dict, id_to_sku: dict) -> list:
    """Run compare_product() for every SKU in PIM."""
    return [
        compare_product(sku, pim_map, storefront_map, id_to_sku)
        for sku in sorted(pim_map)
    ]

def batch_compare_html(pim_map: dict, storefront_map: dict, id_to_sku: dict, output_path: str = "batch_report.html") -> str:
    """Run batch comparison and generate HTML report."""
    results = batch_compare(pim_map, storefront_map, id_to_sku)
